...
`embed-example-nwb.html` shows the same `#fsk` mount switching between frozen JSON and the bundled synthetic NWB excerpt by changing only `source` and `src`. A DANDI URL is documented there as a disabled follow-up target; it is not wired as a working button until remote HDF5 range-open support exists in the adapter.
...
NWB support is currently read-only excerpt playback. It does not include DANDI browsing/search, NWB writing, or broad schema coverage beyond readable acquisition TimeSeries datasets.

The DANDI asset `293b402c-2217-4611-8e68-b66a6b7be3a1` from dandiset `000020` was checked as a future real-file target. Its S3 redirect supports `Range` (`206 Partial Content`) and browser CORS (`Access-Control-Allow-Origin: *`), but the current `NwbSource` opens HDF5 by fetching the full URL into an ArrayBuffer and writing it into the h5wasm filesystem. Remote HDF5 range-open is therefore a TODO, not a working example.
